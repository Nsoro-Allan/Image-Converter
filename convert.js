...
function hideOption(value) {
  [...formatSel.options].forEach((opt) => {
    ...
    if (opt.hidden && opt.selected) {
      ...
      formatSel.value =
        [...formatSel.options].find((o) => !o.hidden)?.value || "";
    }
  });
}
...
function loadFile(file) {
  ...
  qualityWrapper.style.display = needsQualitySlider(currentMime)
    ? "block"
    : "none";
...
form.addEventListener("submit", async (e) => {
  ...
  const supported = testCanvas
    .toDataURL(targetMime)
    .startsWith("data:" + targetMime);

  if (!supported) {
    alert(
      `Sorry, your browser cannot encode images as ${targetMime
        .split("/")[1]
        .toUpperCase()}.`
    );
    return;
